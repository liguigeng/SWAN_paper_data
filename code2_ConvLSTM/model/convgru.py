...
from code2_ConvLSTM.self_dataloader import p, q
# p = 12
# q = 3
...
class ConvGRUNet(nn.Module):
    def __init__(self, inChannel=1, hidden=[32, 32, 32], kernel=[3, 3, 3], outLen=q):
...
if __name__ == '__main__':
    ...
    input = torch.from_numpy(np.random.random((1, 12, 1, 256, 256))).float().to(device)
